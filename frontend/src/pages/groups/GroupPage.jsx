...
function LeaderOverview({ tasks, members }) { const completed = tasks.filter((task) => task.status === 'Completed').length; const completion = tasks.length ? Math.round((completed / tasks.length) * 100) : 0; const priorities = ['High', 'Medium', 'Low']; return <div className="mb-6 grid gap-5 xl:grid-cols-[1fr_1.2fr_0.8fr]"><div className="rounded-lg bg-white p-5 shadow-sm ring-1 ring-slate-200"><div className="flex items-baseline justify-between"><h2 className="text-lg font-black">Completion rate</h2><span className="text-xl font-black text-[#073ca6]">{completion}%</span></div><div className="mt-4 h-3 overflow-hidden rounded-full bg-slate-100"><div className="h-full bg-[#073ca6]" style={{ width: `${completion}%` }}/></div><p className="mt-3 text-sm font-semibold text-slate-500">{completed}/{tasks.length} group tasks completed.</p></div><div className="rounded-lg bg-white p-5 shadow-sm ring-1 ring-slate-200"><h2 className="text-lg font-black">Contribution status</h2><div className="mt-4 grid gap-3">{members.map((member) => { const memberTasks = tasks.filter((task) => String(task.assignedUserId) === String(member.user_id)); const done = memberTasks.filter((task) => task.status === 'Completed').length; const rate = memberTasks.length ? Math.round((done / memberTasks.length) * 100) : 0; return <div key={member.user_id}><div className="flex justify-between gap-2 text-sm"><span className="font-bold">{member.first_name} {member.last_name}</span><span className="font-semibold text-slate-500">{done}/{memberTasks.length} · {rate}%</span></div><div className="mt-1 h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full bg-[#139f98]" style={{ width: `${rate}%` }}/></div></div>; })}</div></div><div className="rounded-lg bg-white p-5 shadow-sm ring-1 ring-slate-200"><h2 className="text-lg font-black">Priority mix</h2><div className="mt-4 grid grid-cols-3 gap-2">{priorities.map((priority) => <div key={priority} className="rounded-md bg-slate-50 p-3"><p className="text-xs font-black text-slate-500">{priority}</p><p className="mt-1 text-2xl font-black">{tasks.filter((task) => task.priority === priority).length}</p></div>)}</div></div></div>; }
